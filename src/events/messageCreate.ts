...
const runCommand = async (
  ...
  if (!access.hasAccess) {
    ...
    await cmd.run(client, message, config, args).catch((err: string) => {
      message.channel.send(
        'Something went wrong, was this command run in the correct place?'
      );
...
export const run: RunFunction = async (
  ...
  if (message.guild) {
    ...
    await UserProfileService.incrementActivityScore(
      message.guild.id,
      message.author.id
    );
